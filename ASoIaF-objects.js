...
function markLocation(map, key, locationOptions) {
  ...
  var url    = (typeof(locationOptions.url)  !="undefined") ? locationOptions.url : "https://awoiaf.westeros.org/index.php/"+key;
...
function markNobility (map, nobilityOptions) {
  ...
  if (typeof(nobilityOptions.wikiKey)=="string") google.maps.event.addListener(marker,"click", function(event) {window.open("https://awoiaf.westeros.org/index.php/"+nobilityOptions.wikiKey, '_blank');});
...
function markPathSegment(map, overlayOptions) {
  if (typeof(overlayOptions.icon)=="string") {
    switch (overlayOptions.icon) {
      ...
      default:      var icon = "https://chart.googleapis.com/chart?chst=d_map_xpin_letter&chld=pin" + overlayOptions.icon;  //or d_map_pin_letter_withshadow
    }
